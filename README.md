...
python -m pipeline scans                  # 6. shrink + encrypt the scans into scans/ (only new or changed pages)
python -m pipeline render                 # 7. build the site into site/
python -m pipeline serve                  # preview at http://localhost:8000 (also reachable from a phone on the same Wi-Fi)
```

`python -m pipeline all` runs steps 2–7. Things to check by hand go to `cache/review.md`. You fix them in `data/overrides.yaml` (merge/split recipes, drop pages, correct a category), then re-run `cluster`, `consolidate` and `render`. Only recipes whose source pages changed call the API again.
...
Every recipe and consejo ends with its page numbers. Open "Ver el original" to see the scans of those pages, so anyone can check the recipe against what is actually written. `scans` writes a 1600 px copy of each page (~150 KB, 44 MB for the whole binder) to `scans/`, encrypted with the family password. That folder is committed, and `render` copies it into the site. The browser only downloads and decrypts the scans when someone opens that section. Run `scans` again after adding or replacing a photo in `pictures/`.
...
Use a long passphrase (4–5 random words). The encrypted files are public, so a short password could be guessed offline. To change the password, update `.env` and the GitHub secret, then run `python -m pipeline lock` and `python -m pipeline scans` and push. Everyone then has to enter the new password.
